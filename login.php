...
if (isset($_POST['confirm']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("select * from application where `man_no`= ?");
    $stmt->bind_param("s", $man);
    $stmt->execute();
    $res = $stmt->get_result()->fetch_assoc();
    if ($res > 0) {
        echo "<p class='text-danger'>This MA no has already applied, duplicate application not acceptable</p>";
    } else {
    ...
    <div class="container">
        <div id="section1" class="form-section active">
            ...
            <form id="section1Form" name="verification" method="post">
                <label>MAN No.</label>
                <input type="text" class="form-control" name="man_no" required value=<?= $_SESSION['man_no'] ?? "" ?>>
